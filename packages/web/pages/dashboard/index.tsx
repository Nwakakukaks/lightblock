...
import {
  isWalletConnected as isLightlinkWalletConnected,
  getWallet as getLightlinkWallet,
  connectWallet as connectLightlinkWallet,
  isProviderAvailable as isLightlinkProviderAvailable,
  isWalletReady as isLightlinkWalletReady,
} from '@modules/blockchains/lightlink/providers/walletProvider';
...
const DashboardPage: NextPage = () => {
  const blockchain = Blockchains.Lightlink;
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const walletConnected = await isLightlinkWalletConnected();
      if (walletConnected) {
        const wallet = await getLightlinkWallet();
  ...
  const handleOnConnectClick = async () => {
    if (!isLightlinkProviderAvailable()) {
      showMessage(
        'Install Metamask',
