...
export const getWallet = async (): Promise<LightLink | null> => {
...
export const getNetwork = (): Networks => {
  return window.ethereum.networkVersion === '1891'
    ? Networks.Testnet
    : Networks.Mainnet;
};
...
export const getExplorerAddressUrl = (
  ...
  if (environment === Environments.Testnet) {
    return `https://pegasus.lightlink.io/address/${hash}`;
  }
  if (environment === Environments.Mainnet) {
    return `https://phoenix.lightlink.io/address/${hash}`;
  }
  return null;
};
...
export const getExplorerTxUrl = (
  ...
  if (environment === Environments.Testnet) {
    return `https://pegasus.lightlink.io/tx/${hash}`;
  }
  if (environment === Environments.Mainnet) {
    return `https://phoenix.lightlink.io/tx/${hash}`;
  }
  return null;
};
